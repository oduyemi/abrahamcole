...
export const Header = ({ showHeader, isBlackBackground }) => {
  ...
  return (
    <header>
       <nav id="header" className={`bg-transparent main_header ${isBlackBackground ? 'white-text' : 'black-text'}`}>
        <div className=" mt-0 py-1 flex items-center justify-between">
          ...
          <ul className={`nav-menu-wrapper flex-col md:flex-row flex md:space-x-8 mt-4 md:mt-0 md:text-l md:font-medium`} id="mobile-menu">
            <li>
                <Link to="/selected-projects" className="text-gray-700 hover:bg-gray-50 border-b border-gray-100 md:hover:bg-transparent md:border-0 block pl-3 pr-4 py-2 md:hover:text-butter md:p-0">
          ...
          <div className='mx-auto logo'>
            <Link to="/" className="text-l block py-2 hover:text-butter">
              <img src={isBlackBackground ? sitelogoWhite : sitelogo} alt="logo" width="70%"></img>
            </Link>
          </div>
        </div>
      </nav>
